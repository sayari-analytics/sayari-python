...
class TagEnum(str, enum.Enum):
    ...
    def visit(
        ...
        if self is TagEnum.PEP:
            return pep()
        if self is TagEnum.STATE_OWNED:
            return state_owned()
        if self is TagEnum.FORMER_SOE:
            return former_soe()
        if self is TagEnum.SANCTIONED:
            return sanctioned()
        if self is TagEnum.FORMERLY_SANCTIONED:
            return formerly_sanctioned()
        if self is TagEnum.REPUTATIONAL_RISK_TERRORISM:
            return reputational_risk_terrorism()
        if self is TagEnum.REPUTATIONAL_RISK_ORGANIZED_CRIME:
            return reputational_risk_organized_crime()
        if self is TagEnum.REPUTATIONAL_RISK_FINANCIAL_CRIME:
            return reputational_risk_financial_crime()
        if self is TagEnum.REPUTATIONAL_RISK_BRIBERY_AND_CORRUPTION:
            return reputational_risk_bribery_and_corruption()
        if self is TagEnum.REPUTATIONAL_RISK_OTHER:
            return reputational_risk_other()
        if self is TagEnum.REPUTATIONAL_RISK_CYBERCRIME:
            return reputational_risk_cybercrime()
        if self is TagEnum.REPUTATIONAL_RISK_MODERN_SLAVERY:
            return reputational_risk_modern_slavery()
        if self is TagEnum.REGULATORY_ACTION:
            return regulatory_action()
        if self is TagEnum.LAW_ENFORCEMENT_ACTION:
            return law_enforcement_action()
        if self is TagEnum.EXPORT_CONTROLS:
            return export_controls()
        if self is TagEnum.FORCED_LABOR_XINJIANG_CONTRACTORS:
            return forced_labor_xinjiang_contractors()
        if self is TagEnum.WRO_ENTITY:
            return wro_entity()
        if self is TagEnum.UFLPA_ENTITY:
            return uflpa_entity()
        if self is TagEnum.SHEFFIELD_HALLAM_UNIVERSITY_FORCED_LABOR_ENTITY:
